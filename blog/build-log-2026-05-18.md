...
`README.md`, `doc/PRODUCT.md`, and `doc/SPEC.md` were updated to reflect shipped capabilities: deliverables/work products are no longer listed as roadmap-only, the Deliverables section covers both file artifacts and issue-linked documents, and the operator UI section mentions the issue pipeline view. Docs-only, no behaviour changes. Contributed by @ralphbibera.
...
**UI test execution for the graph view was not fully confirmed.** An existing `jsdom`/ESM issue under Node 19 blocked full Vitest execution for the UI test file in the development environment. Backend and focused UI tests were added, but the full UI suite wasn't run locally.
