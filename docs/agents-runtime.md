...
Audience: Operators setting up and running agents in Bizbox

## 1. What this system does

Agents in Bizbox do not run continuously.  
...
For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), Bizbox assumes the CLI is already installed and authenticated on the host machine.
...
Bizbox stores session IDs for resumable adapters.
...
Bizbox pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Bizbox surfaces this as a warning in environment tests, not a hard error.
