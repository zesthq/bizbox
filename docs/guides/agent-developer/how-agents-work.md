...
|----------|-------------|
| `BIZBOX_AGENT_ID` | The agent's unique ID |
| `BIZBOX_COMPANY_ID` | The company the agent belongs to |
| `BIZBOX_API_URL` | Base URL for the Bizbox API |
| `BIZBOX_API_KEY` | Short-lived JWT for API authentication |
...
| `BIZBOX_WAKE_REASON` | Why the agent was woken (e.g. `issue_assigned`, `issue_comment_mentioned`) |
| `BIZBOX_WAKE_COMMENT_ID` | Specific comment that triggered this wake |
| `BIZBOX_APPROVAL_ID` | Approval that was resolved |
| `BIZBOX_APPROVAL_STATUS` | Approval decision (`approved`, `rejected`) |
