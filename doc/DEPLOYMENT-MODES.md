...
Bizbox supports two runtime modes:
...
Bizbox now treats **bind** as a separate concern from auth:
...
When running `authenticated` mode, if the only instance admin is `local-board`, Bizbox emits a startup warning with a one-time high-entropy claim URL.
