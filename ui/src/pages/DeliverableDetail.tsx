...
function DetailSidePanel({ data }: { data: DeliverableDetailType }) {
  const showWorkflow = data.sourceKind === "workflow" && data.workflow !== null;
  const showRoot = data.childIssue !== null && data.rootIssue !== null && data.rootIssue.id !== data.childIssue.id;
  return (
    <aside className="space-y-4 rounded-md border border-border bg-card p-4">
      ...
      {showWorkflow && data.workflow ? (
        <DetailRow label="Workflow">
          <Link to={`/workflows/${data.workflow.id}`} className="hover:underline">
            <span className="text-foreground">{data.workflow.title}</span>
          </Link>
          <div className="text-[11px] text-muted-foreground">
            Run {data.workflow.runId}
          </div>
        </DetailRow>
      ) : null}
