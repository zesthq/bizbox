...
function renderApiAccessNote(env: Record<string, string>): string {
  if (!hasNonEmptyEnvValue(env, "BIZBOX_API_URL") || !hasNonEmptyEnvValue(env, "BIZBOX_API_KEY")) return "";
  return [
    "Paperclip API access note:",
    "Use run_shell_command with curl to make Paperclip API requests.",
    "GET example:",
    `  run_shell_command({ command: "curl -s -H \\"Authorization: Bearer $BIZBOX_API_KEY\\" \\"$BIZBOX_API_URL/api/agents/me\\"" })`,
    "POST/PATCH example:",
    `  run_shell_command({ command: "curl -s -X POST -H \\"Authorization: Bearer $BIZBOX_API_KEY\\" -H 'Content-Type: application/json' -H \\"X-Paperclip-Run-Id: $BIZBOX_RUN_ID\\" -d '{...}' \\"$BIZBOX_API_URL/api/issues/{id}/checkout\\"" })`,
    "",
    "",
  ].join("\n");
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (effectiveWorkspaceCwd) env.BIZBOX_WORKSPACE_CWD = effectiveWorkspaceCwd;
