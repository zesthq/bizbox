...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const query = joinPromptSections([instructionsPrefix, wakePrompt, sessionHandoffNote, renderedPrompt]);
  ...
  const adkRoot = path.join(paperclipHome(), "adk", "companies", agent.companyId, "agents", agent.id);
