...
const TAILNET_BIND_WARNING =
  "No Tailscale address was detected during setup. The saved config will stay on loopback until Tailscale is available or BIZBOX_TAILNET_BIND_HOST is set.";

const ONBOARD_ENV_KEYS = [
  "BIZBOX_PUBLIC_URL",
  "DATABASE_URL",
  "BIZBOX_DB_BACKUP_ENABLED",
  "BIZBOX_DB_BACKUP_INTERVAL_MINUTES",
  "BIZBOX_DB_BACKUP_RETENTION_DAYS",
  "BIZBOX_DB_BACKUP_DIR",
  "BIZBOX_DEPLOYMENT_MODE",
  "BIZBOX_DEPLOYMENT_EXPOSURE",
  "BIZBOX_BIND",
  "BIZBOX_BIND_HOST",
  "BIZBOX_TAILNET_BIND_HOST",
  "HOST",
  "PORT",
  "SERVE_UI",
  "BIZBOX_ALLOWED_HOSTNAMES",
  "BIZBOX_AUTH_BASE_URL_MODE",
  "BIZBOX_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "BIZBOX_STORAGE_PROVIDER",
  "BIZBOX_STORAGE_LOCAL_DIR",
  "BIZBOX_STORAGE_S3_BUCKET",
  "BIZBOX_STORAGE_S3_REGION",
  "BIZBOX_STORAGE_S3_ENDPOINT",
  "BIZBOX_STORAGE_S3_PREFIX",
  "BIZBOX_STORAGE_S3_FORCE_PATH_STYLE",
  "BIZBOX_SECRETS_PROVIDER",
  "BIZBOX_SECRETS_STRICT_MODE",
  "BIZBOX_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(opts?: { preferTrustedLocal?: boolean }): {
  ...
  const publicUrl = preferTrustedLocal
    ? undefined
    : (
      process.env.BIZBOX_PUBLIC_URL?.trim() ||
      process.env.BIZBOX_AUTH_PUBLIC_BASE_URL?.trim() ||
  ...
  const deploymentMode = preferTrustedLocal
    ? "local_trusted"
    : (parseEnumFromEnv<DeploymentMode>(process.env.BIZBOX_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted");
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.BIZBOX_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  const deploymentExposure =
    deploymentMode === "local_trusted" ? "private" : (deploymentExposureFromEnv ?? "private");
  const bindFromEnv = parseEnumFromEnv<BindMode>(process.env.BIZBOX_BIND, BIND_MODES);
  const customBindHostFromEnv = process.env.BIZBOX_BIND_HOST?.trim() || undefined;
  ...
  const resolvedBind = resolveRuntimeBind({
    ...
    tailnetBindHost: process.env.BIZBOX_TAILNET_BIND_HOST?.trim(),
  });
  const authPublicBaseUrl = publicUrl;
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.BIZBOX_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.BIZBOX_ALLOWED_HOSTNAMES
    ? process.env.BIZBOX_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.BIZBOX_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.BIZBOX_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.BIZBOX_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.BIZBOX_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.BIZBOX_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.BIZBOX_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.BIZBOX_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.BIZBOX_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.BIZBOX_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.BIZBOX_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.BIZBOX_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.BIZBOX_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.BIZBOX_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.BIZBOX_SECRETS_MASTER_KEY_FILE) ??
  ...
  if (deploymentMode === "local_trusted" && process.env.BIZBOX_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "BIZBOX_DEPLOYMENT_EXPOSURE",
      reason: "Ignored because deployment mode local_trusted always forces private exposure",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.BIZBOX_BIND !== undefined) {
    ignoredEnvKeys.push({
      key: "BIZBOX_BIND",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.BIZBOX_BIND_HOST !== undefined) {
    ignoredEnvKeys.push({
      key: "BIZBOX_BIND_HOST",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  if (existingConfig) {
    ...
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("BIZBOX_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.BIZBOX_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("BIZBOX_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("BIZBOX_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: BIZBOX_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.BIZBOX_OPEN_ON_LISTEN = "true";
